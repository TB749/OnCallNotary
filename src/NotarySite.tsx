...
export default function NotarySite() {
  ...
  const DocumentIcon = (props: SVGProps<SVGSVGElement>) => (
    ...
    >
      <rect
        ...
        fill="none"
      />
      <path d="M17 16h12M17 22h12M17 28h8" strokeLinecap="round" />
      <path
        d="M33 18v20l6-4 6 4V18A6 6 0 0 0 33 18Z"
        fill="none"
        stroke="currentColor"
      />
    </svg>
  ...
  const RibbonIcon = (props: SVGProps<SVGSVGElement>) => (
    <svg
      ...
      {...props}
    >
      <circle cx="20" cy="18" r="10" fill="none" stroke="currentColor" />
      <path
        d="M12 30v14l8-4 8 4V30"
        fill="none"
        stroke="currentColor"
        strokeLinejoin="round"
      />
      <path d="M20 8V4" strokeLinecap="round" />
      <path d="M28.5 10.5 31 8" strokeLinecap="round" />
      <path d="M11.5 10.5 9 8" strokeLinecap="round" />
    </svg>
  );

  useEffect(() => {
    const logoSrc = withBase("logo-small.png");
    console.assert(
    ...
    console.assert(!!document.querySelector("#book"), "Booking section exists");
    console.assert(
      !!document.querySelector("#compliance"),
      "Compliance section exists"
    );
    console.assert(!!document.querySelector("#faq"), "FAQ section exists");
    console.assert(
      (document.querySelectorAll("#faq details").length || 0) > 0,
      "FAQ has at least one item"
    );
    console.assert(
    ...
    console.assert(
      !!document.querySelector("#calendly-inline"),
      "Calendly container exists"
    );
    console.assert(
      !!document.querySelector(`img[src="${logoSrc}"]`) ||
        !!document.querySelector('img[alt*="Notary logo"]'),
      "Logo image present or fallback"
    );
    console.assert(
      !!document.querySelector('header a[href="#book"]'),
      "Header Booking link exists"
    );
    console.assert(!!document.querySelector("footer"), "Footer exists");
    console.assert(
      !!document.querySelector('a[href="#pricing"]'),
      "Nav link to Pricing exists"
    );
    console.assert(
      (document.body.textContent || "").includes(
  ...
  useEffect(() => {
    const loadCalendly = () => {
      ...
      if (Calendly && container && !container.dataset.initialized) {
        Calendly.initInlineWidget({
          url: "https://calendly.com/oncall-notaryontario",
    ...
    if (existing) {
      if ((window as any).Calendly) loadCalendly();
      else existing.addEventListener("load", loadCalendly, { once: true });
    } else {
      const s = document.createElement("script");
      s.src = "https://assets.calendly.com/assets/external/widget.js";
      s.async = true;
      s.onload = loadCalendly;
      head.appendChild(s);
    }

    const mo = new MutationObserver(() => {
      const iframe = document.querySelector("#calendly-inline iframe");
      if (iframe) {
        setInlineLoaded(true);
        const hardcoded = Number.parseInt(
          iframe.getAttribute("height") || "",
          10
        );
        if (Number.isFinite(hardcoded)) {
          setCalendlyHeight((prev) => Math.max(prev, hardcoded + 40));
        }
      }
    });

    const target = document.getElementById("calendly-inline");
  ...
  useEffect(() => {
    const handleCalendlyMessage = (event: MessageEvent) => {
      ...
      if (typeof eventName !== "string" || !eventName.startsWith("calendly."))
        return;
      const payload = (data as { payload?: { height?: unknown } }).payload;
      const incomingHeight = payload?.height;
      let numericHeight =
        typeof incomingHeight === "number" ? incomingHeight : NaN;
      if (!Number.isFinite(numericHeight) && incomingHeight != null) {
        const parsed = parseFloat(String(incomingHeight));
        numericHeight = Number.isFinite(parsed) ? parsed : NaN;
      }

      if (Number.isFinite(numericHeight) && numericHeight > 0) {
        setCalendlyHeight((prev) =>
          Math.max(prev, Math.ceil(numericHeight + 40))
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const applyHeight = (node: HTMLElement) => {
      node.style.minHeight = `${calendlyHeight}px`;
      node.style.height = `${calendlyHeight}px`;
    };
        ...
        <section className="bg-[#f1e4d3] px-4 py-16 md:py-24">
          <div className="mx-auto flex max-w-6xl flex-col gap-12 md:flex-row md:items-start md:justify-between">
            <div className="w-full max-w-xl space-y-8">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#7d6650]">
                  Book an appointment
                </p>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="rounded-3xl bg-[#fdf7ee] p-6 shadow-sm ring-1 ring-[#d3bfa4]/40">
                  <DocumentIcon className="h-12 w-12 text-[#8c7046]" />
                  <p className="mt-4 text-sm font-semibold text-[#2d1b0f]">
                ...
                <div className="rounded-3xl bg-[#fdf7ee] p-6 shadow-sm ring-1 ring-[#d3bfa4]/40">
                  <RibbonIcon className="h-12 w-12 text-[#8c7046]" />
                  <p className="mt-4 text-sm font-semibold text-[#2d1b0f]">
                    Notary & True Copies
                  </p>
                  <p className="mt-1 text-sm text-[#5c4634]">
                    Certified copies, witnessing, mobile notarization.
                  </p>
                </div>
              </div>
            </div>
            <div id="legacy-calendar" className="w-full max-w-md">
                ...
                {!inlineLoaded && (
                  <div className="absolute inset-0 grid place-items-center bg-white">
                    <div className="text-center">
                      <div className="mx-auto mb-3 h-10 w-10 rounded-full border-2 border-[#d8c6aa] border-t-transparent animate-spin" />
                      <p className="text-sm text-[#7d6650]">
                        Loading scheduler…
                      </p>
                    </div>
                  </div>
              ...
              <p className="mt-4 text-xs text-[#7d6650]">
                Prefer email? Contact{" "}
                <a
                  className="font-semibold text-[#0b2b4a]"
