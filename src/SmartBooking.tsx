...
export default function SmartBooking() {
  ...
  const modes = allowedModes(form.serviceId, estimateTime(form.timing), form.operation);
  ...
  const set = (key:keyof Form, value:string|number|boolean) => {
    setForm(current => {
      ...
      if (['serviceId', 'timing', 'operation'].includes(key)) {
        ...
        const eligible = allowedModes(next.serviceId, estimateTime(next.timing), next.operation);
  ...
  const next = () => {
    if (step === 0 && service?.operations.includes(form.operation) && form.timing && modes.length === 0) {
      setForm({...initial});
      setStep(0);
      setQuote(null);
      setScheduling(false);
      setErrors({});
      setMessage('Booking restarted. Please choose a service and an available appointment period.');
      heading.current?.focus();
      heading.current?.scrollIntoView({behavior:'smooth', block:'start'});
      return;
    }
    const allErrors = validateDetails(form);
    const currentErrors = step === 0 ? Object.fromEntries(Object.entries(allErrors).filter(([key]) => ['serviceId', 'operation', 'timing', 'mode', 'platform'].includes(key))) : allErrors;
  ...
  return <div className="smart-booking" id="smart-booking"><p className="sb-eyebrow">Smart Booking</p>
    ...
    <form noValidate onSubmit={event => {event.preventDefault(); step === 2 ? schedule() : next();}}>
      ...
      {step === 0 && <>
        ...
        {form.serviceId && form.operation && <><label htmlFor="sb-timing">Appointment period *</label><select {...field('timing')} value={form.timing} onChange={event => set('timing', event.target.value)}><option value="">Choose a period</option><option value="regular">Before 6 PM · Office hours</option><option value="urgent">From 6 PM · Urgent</option></select>{error('timing')}<p className="sb-help">America/Toronto. This period determines your estimate and eligible format. Calendly supplies the actual available dates and times. Your selected period is a request; check the event time before booking.</p></>}
        {form.timing && form.serviceId && form.operation && <><fieldset><legend>Appointment format *</legend><div className="sb-options">{modes.map(mode => <label key={mode} className={form.mode === mode ? 'selected' : ''}><input type="radio" name="mode" checked={form.mode === mode} onChange={() => set('mode', mode)}/>{MODES[mode as keyof typeof MODES]}</label>)}</div>{!modes.length && <p>This service requires physical attendance. Choose office hours.</p>}</fieldset>{error('mode')}{urgent && <p className="sb-help">Online commissioning uses double the regular service price. Document drafting keeps the same per-document price before and after 6 PM. After-hours mediation requires review.</p>}</>}
      ...
      {step === 1 && <>
        ...
        {form.serviceId === 'drafting' && <p className="sb-help">Affidavit $65 per document · Travel Consent Letter $50 per document, before and after 6 PM. Prices exclude HST. Commissioning and notarization are separate services.</p>}
