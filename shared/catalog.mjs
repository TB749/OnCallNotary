...
export const SERVICES = [
 ...
 {id:'drafting',title:'Legal Document Drafting',blurb:'Affidavit and travel consent letter drafting. Commissioning and notarization are charged separately.',image:'/services/legal-drafting.png',points:['Draft Affidavit — $65 per document, any time','Travel Consent Letter — $50 per document, any time'],operations:['draft-affidavit','draft-consent']},
];
...
export function allowedModes(serviceId,time,operation='') {
 if(OPERATIONS[operation]?.online === false) return urgentFor(time)?[]:['in_person','mobile'];
...
export function quoteBooking(data,settings=pricingSettings()) {
 ...
 if(!Number.isInteger(quantity)||quantity<1||quantity> (op.unit==='hour'?8:500)) throw new Error('Enter a valid whole-number quantity.');
  if(op.unit==='quote') {
   reasons.push('This document needs an individual scope and price review.');
 } else {
   let amount=op.unit==='seal'
     ? (pages
       ? quantity*(quantity>=50?300:quantity>=15?400:1500)
       : op.first+(quantity-1)*op.additional)
     : op.first*quantity;

   lines.push({
     label:op.label,
     quantity,
     unit:pages?'page':op.unit,
     amount
   });
   ...
   if(urgentFor(data.time) && !op.afterHoursRegular) {
     if(settings.urgentOperations.includes(data.operation)) {
       lines.push({
         label:'Urgent service adjustment (2× regular service total)',
         quantity:1,
         unit:'adjustment',
         amount
       });
     } else {
       reasons.push('After-hours eligibility and pricing require owner approval for this service.');
     }
   }
 }

 if(data.mode==='mobile') {
   reasons.push('Travel is $1.50/km. The billable distance and service location must be verified before a final total is issued.');
 }

 return {
   status:reasons.length?'review_required':'calculated',
   lines,
   reasons,
   ...taxTotals(lines.reduce((sum,x)=>sum+x.amount,0)),
   billingUnit:pages?'page':op.unit,
   notes:[
     '13% HST. The 50% deposit is requested only after confirmation.',
     'Final scope and appointment availability require confirmation.'
   ]
 };
}
export function validateBooking(input, now=new Date()) {
 ...
 if(!allowedModes(data.serviceId,data.time,data.operation).includes(data.mode))errors.mode='This format is not available for the selected time, service and operation.';
